# Capability: bio.tools Upload

## Purpose
Define the end-to-end upload capability for adding new tools to the bio.tools registry via the CLI, covering authentication, pre-flight existence checks, POST creation, retry and resume behavior, opt-in configuration, and reporting (JSONL + CSV with API URLs).
## Requirements
